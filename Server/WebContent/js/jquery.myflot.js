...
(function($) {
	...
	Canvas.prototype.getTextLayer = function(classes) {
		...
		if (layer == null) {
			layer = this.text[classes] = $("<div></div>").addClass(
					"flot-text " + classes).css({
				position : "absolute",
				top : 0,
				left : 0,
				bottom : 0,
	...
	Canvas.prototype.addText = function(layer, x, y, text, font, angle, halign,
			valign) {
	...
	function Plot(placeholder, data_, options_, plugins) {
		...
		var series = [], options = {
			...
			xaxis : {
				...
				timezone : null, // "browser" for local to the client or
									// timezone for timezone-js
				font : null, // null (derived from CSS in placeholder) or
								// object like { size: 11, style: "italic",
		...
		function parseOptions(opts) {
			...
			for (i = 0; i < axisCount; ++i) {
				axisOptions = $.extend(true, {}, options.xaxis,
						options.xaxes[i]);
			...
			for (i = 0; i < axisCount; ++i) {
				axisOptions = $.extend(true, {}, options.yaxis,
						options.yaxes[i]);
		...
		function measureTickLabels(axis) {
			...
			var opts = axis.options, ticks = axis.ticks || [], axisw = opts.labelWidth || 0, axish = opts.labelHeight || 0, legacyStyles = axis.direction
		...
		function setupTickGeneration(axis) {
			...
			if (!axis.tickGenerator) {
				...
				axis.tickGenerator = function(axis) {
					var maxDec = opts.tickDecimals, dec = -Math.floor(Math
							.log(axis.delta)
							/ Math.LN10);
					...
					var magn = Math.pow(10, -dec), norm = axis.delta / magn, // norm
																				// is
																				// between
																				// 1.0
																				// and
					...
					ticks = [], start, i = 0, v = Number.NaN, prev;
					...
					axis.tickDecimals = Math.max(0, maxDec != null ? maxDec
							: dec);
		...
		function drawSeriesLines(series) {
			function plotLine(datapoints, xoffset, yoffset, axisx, axisy) {
				...
				ctx.beginPath();
				for ( var i = 0; i < points.length; i += 2*ps) {
					var x1 = points[i], y1 = points[i + 1],x2 = points[i+ps], y2 = points[i + ps+1];
			//	for ( var i = 0; i < points.length; i += 4) {
			//		var x1 = points[i+2], y1 = points[i +3], x2 = points[i], y2 = points[i + 1];
	...
	$.myplot = function(placeholder, data, options) {
		// var t0 = new Date();
		var plot = new Plot($(placeholder), data, options, $.myplot.plugins);
		// (window.console ? console.log : alert)("time used (msecs): " + ((new
		// Date()).getTime() - t0.getTime()));
		return plot;
	};

	$.myplot.version = "0.8.0-beta";
